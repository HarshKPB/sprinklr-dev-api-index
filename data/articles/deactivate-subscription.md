...
slug: deactivate-subscription
url: https://dev.sprinklr.com/deactivate-subscription
---
...
You can deactivate a Webhook Subscription in the Sprinklr UI via this API call and you will get the response based on Success.

## API Endpoint

 https://api3.sprinklr.com/{env}/api/v2/webhook-subscriptions/deactivate
...
Add a Request Body in POST request. In the body, enter a raw request with the format type as JSON since we have to send the correct format according to the server. This endpoint expects a JSON body which contains the details of the Create Webhook endpoint. **Request Parameters**
...
| addSubscriptions | Optional | Add subscriptions to the list of subscribed sprinklr webhook types | String |
| removeSubscriptions | Optional | Remove subscriptions from the list of subscribed sprinklr webhook types. | String |
...
## Example
  ...
  Copy Code
	...
	curl -X POST \
	 https://api2.sprinklr.com/{env}/api/v2/webhook-subscriptions/{subscriptionId}/deactivate \
...
[](https://dev.sprinklr.com/deactivate-subscription)

[Back to top](https://dev.sprinklr.com/deactivate-subscription)
